...
\begin{figure}[htpb]
    ...
    \caption{Arquitectura general del sistema de seguimiento visual, válida tanto para simulación como para despliegue real.}
    \label{fig:arquitectura_nodos}
\end{figure}

\section{Componente de Percepción}

\subsection{Entrenamiento y Comparación de Modelos YOLO}

DECIR DATA AUG y  omo lo entrene finetuning

Para seleccionar el modelo de detección se compararon distintas variantes de YOLO atendiendo a tres criterios principales: precisión de detección, precisión media en distintos umbrales de IoU y tiempo de inferencia. Todos los modelos fueron entrenados utilizando el dataset final descrito en la subsección anterior, manteniendo las mismas condiciones de entrenamiento y empleando un total de 25 épocas. De este modo, la comparación entre arquitecturas se realizó de forma homogénea, evitando que las diferencias observadas dependieran de cambios en el conjunto de datos o en la configuración experimental. La Tabla \ref{tab:comparativa-yolo} resume los resultados obtenidos para cada arquitectura evaluada.
...
\begin{equation}
\omega_{yaw} = \mathrm{sat}_{[-20,20]}\left(
...
\begin{equation}
v_y = \mathrm{sat}_{[-10,10]}\left(
2.65\,e_{y,virtual}(t)
+ 0.03 \int_0^t e_{y,virtual}(\tau)\,d\tau
+ 0.192\,\frac{de_{y,virtual}(t)}{dt}
\right)
\end{equation}
...
0.47\,e_{dist}(t)
+ 0.02 \int_0^t e_{dist}(\tau)\,d\tau
+ 0.0014\,\frac{de_{dist}(t)}{dt}
\right)
\end{equation}
...
\hline
PID de \textit{yaw} & \texttt{ex\_virtual} & \texttt{w\_yaw} & 2.365625 & 0.06 & 0.320 & $\pm 20.0$ rad/s \\
\hline
PID de altura & \texttt{ey\_virtual} & \texttt{v\_y} & 2.65 & 0.03 & 0.192 & $\pm 10.0$ m/s \\
\hline
PID de distancia & \texttt{e\_dist} & \texttt{v\_x} & 0.47 & 0.02 & 0.0014 & $\pm 10.0$ m/s \\
\hline
\end{tabular}%
}
...
\section{Ejecución en Gazebo}

\subsection{Lanzamiento de la Simulación}
