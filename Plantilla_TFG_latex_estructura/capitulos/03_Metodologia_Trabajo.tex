...
Por este motivo, en este trabajo se opta por un detector de la familia YOLO. A diferencia de los enfoques de dos etapas, YOLO realiza la detección en una única pasada sobre la imagen, proporcionando directamente la caja delimitadora del UAV objetivo. Esta salida resulta suficiente para la metodología propuesta, ya que a partir de ella se obtiene el centro del objetivo, el error visual respecto al centro de la cámara y el área normalizada utilizada como referencia aproximada de distancia. Así, YOLO ofrece un compromiso adecuado entre rapidez, simplicidad de integración y precisión para una tarea de seguimiento visual de UAVs .
...
explicar como he hehco el dataset y como lo he entrnado finetuning de yolo
\subsection{Geometría Proyectiva aplicada a la Percepción Visual}

\vspace{0.3cm}

La geometría proyectiva se incorpora en esta metodología para justificar cómo se interpretan las medidas obtenidas por el detector visual. En este trabajo, YOLO no proporciona directamente la posición tridimensional del UAV objetivo, sino una caja delimitadora en la imagen. A partir de esa caja se calculan el centro del objetivo, el error respecto al centro del fotograma y el área normalizada. Por tanto, esta subsección no se plantea como un bloque independiente de reconstrucción 3D, sino como la base geométrica que permite entender por qué esas magnitudes visuales son válidas para el seguimiento.
...
En esta expresión, $X_c$ e $Y_c$ representan la posición lateral y vertical del punto respecto al eje óptico, $Z_c$ representa la profundidad y $f$ es la distancia focal. Esta relación conecta directamente con lo realizado en el módulo de percepción: si el UAV objetivo se desplaza lateralmente o verticalmente, cambia el centro de la caja detectada; si se acerca o se aleja, cambia su tamaño aparente. Por esta razón, el error visual y el área normalizada permiten alimentar un seguimiento reactivo sin reconstruir completamente la posición 3D del objetivo \cite{bergamasco2019pinhole, CameraModels}.
...
Por este motivo, se optó por convertir el tamaño de la caja en una distancia aproximada expresada en metros. Esta decisión permite que el PID de distancia trabaje con una variable física, comparando la distancia estimada con una distancia deseada real. La ventaja de este planteamiento es que el controlador queda desacoplado del sensor concreto: si en el futuro se incorpora otro sistema capaz de medir distancia en metros, el PID de distancia no tendría que cambiar, ya que seguiría recibiendo una entrada con las mismas unidades.
...
\begin{itemize}
    ...
    \item $X_c$ e $Y_c$ representan las dimensiones reales conocidas del UAV objetivo. En este trabajo se considera un ancho aproximado de $X_c = 0.55\,\mathrm{m}$ y una altura aproximada de $Y_c = 0.11\,\mathrm{m}$.
