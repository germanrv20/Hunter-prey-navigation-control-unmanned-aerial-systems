...
La adopción del PID en este Trabajo de Fin de Grado responde, además, a una razón práctica importante. El sistema debe ejecutarse sobre una plataforma embebida que también dedica recursos al procesamiento visual y a la inferencia en tiempo real. En este contexto, un controlador PID ofrece una solución eficiente y menos costosa en términos computacionales que otros enfoques más complejos, manteniendo al mismo tiempo una respuesta reactiva robusta y una integración sencilla con los lazos internos del autopiloto. Por ello, resulta una elección especialmente adecuada para transformar el error visual del objetivo en comandos de movimiento del UAV cazador.

\vspace{0.3cm}

\section{Estimación de Estado y Filtrado}

El rendimiento del controlador descrito en la sección anterior depende directamente de la calidad de la señal visual que recibe. En navegación basada en visión, la posición observada del objetivo no constituye una medida exacta, sino una estimación afectada por ruido, discretización, pérdidas ocasionales y variaciones entre fotogramas. Si estas observaciones se introducen de forma directa en el lazo de control, pueden provocar acciones de mando erráticas y reducir la estabilidad del seguimiento.

\vspace{0.3cm}

En este contexto, la estimación de estado y el filtrado actúan como un bloque intermedio entre la percepción y el control. Su función es obtener una representación temporal más consistente del movimiento del blanco, atenuar fluctuaciones  y proporcionar una señal de error más robusta para el controlador PID. De este modo, el sistema no solo reacciona a lo que la cámara mide en cada instante, sino también a una versión depurada  de dicha información. En los apartados siguientes se presentarán dos herramientas especialmente relevantes para este propósito: el filtro de Kalman, orientado a la estimación y predicción del estado, y el filtro EMA, empleado como mecanismo sencillo de suavizado temporal.
...
\begin{center}
    ...
    {\small \textbf{Figura \thefigure.}Ejemplo uso de Filtro de Kalman. }
...
donde $x_k$ representa la medida actual, $\hat{x}_k$ la salida filtrada y $\hat{x}_{k-1}$ el valor filtrado del instante anterior. El parámetro $\alpha$, con $0 < \alpha \leq 1$, determina el peso relativo entre la muestra nueva y el historial previo \cite{industrialEma, mciEma}.
...
Esta segunda forma resulta útil para interpretar el funcionamiento del filtro: la nueva salida se obtiene corrigiendo el valor filtrado anterior en función de la diferencia entre la medida actual y la estimación previa. Si $\alpha$ toma valores pequeños, el suavizado es mayor y la respuesta del filtro es más lenta; si $\alpha$ se aproxima a 1, la salida sigue más de cerca la medida y el efecto de suavizado disminuye \cite{industrialEma, mciEma, wikipediaSuavizamientoExponencial}.
...
En consecuencia, el EMA constituye una técnica de suavizado sencilla, ligera y eficiente para señales discretas. Su bajo coste computacional y su formulación recursiva hacen que resulte especialmente útil en aplicaciones donde se requiere atenuar ruido de alta frecuencia sin introducir una complejidad elevada \cite{industrialEma, mciEma}.
