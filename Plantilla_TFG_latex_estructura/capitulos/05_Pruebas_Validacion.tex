...
La validación del sistema se estructura como una secuencia progresiva de pruebas, diseñada para comprobar primero los elementos individuales y, posteriormente, el comportamiento del sistema completo de seguimiento visual. En las secciones siguientes se evalúan tanto la percepción basada en YOLO como la respuesta de los controladores PID, la continuidad del seguimiento en simulación y una primera comprobación real del sistema de estimación de distancia.

\vspace{0.3cm}

Antes de presentar los experimentos se definen las métricas utilizadas durante todo el capítulo. Para la selección del detector se consideran el número de parámetros, el tiempo de inferencia, el \textit{mAP50} y el \textit{mAP50-95}. Para las pruebas de control se emplean los errores normalizados de centrado en imagen, el EPE 2D, el RMSE de distancia y el tiempo de establecimiento. Finalmente, en las pruebas de seguimiento se añaden métricas de continuidad visual, diferenciando entre frames con detección directa de YOLO, frames mantenidos mediante Kalman y pérdidas completas del objetivo mediante el TLR.

\vspace{0.3cm}

El primer bloque experimental se dedica a la selección del modelo de detección. Para ello se realiza un torneo comparativo entre distintas arquitecturas de la familia YOLO, incluyendo YOLOv8, YOLOv10, YOLO11 y YOLO26 en sus variantes \textit{nano}, \textit{small} y \textit{medium}. Todas las alternativas se entrenan bajo condiciones comunes y se comparan en términos de precisión, tamaño del modelo y velocidad de inferencia. El objetivo es justificar la elección del detector que ofrece el mejor equilibrio para integrarse en un lazo de control visual en tiempo real.

\vspace{0.3cm}

El segundo bloque se centra en la validación de los controladores PID. Primero se prueban de forma individual los tres reguladores principales: el PID de \textit{yaw}, mediante una trayectoria circular; el PID de altura, mediante una trayectoria vertical sinusoidal; y el PID de distancia, mediante una trayectoria recta con separación de seguridad. Después se estudia el tiempo de establecimiento de cada controlador por separado, con el fin de cuantificar la rapidez con la que cada eje reduce su error. Finalmente, se realiza una prueba conjunta con los tres PID activados simultáneamente, donde el UAV objetivo combina desplazamiento circular y variación de altura para comprobar que el sistema completo mantiene una respuesta coordinada y estable.

\vspace{0.3cm}

El tercer bloque evalúa el seguimiento del UAV presa en trayectorias más exigentes. En primer lugar se analiza una trayectoria en zigzag, útil para comprobar la corrección de cambios laterales sucesivos y la estabilidad de la detección. A continuación se reutiliza la trayectoria circular ondulada de la prueba conjunta de PID para medir el seguimiento cuando aparecen errores simultáneos en \textit{yaw}, altura y distancia. Por último, se incorpora una ruta final definida por \textit{waypoints}, con desplazamientos más largos y cambios de altura, para cerrar la validación en simulación con una maniobra tridimensional menos regular y más representativa de un escenario general de persecución.

\vspace{0.3cm}

El cuarto bloque corresponde a las pruebas reales. En esta parte se valida la estimación de distancia utilizando la cámara real y el UAV objetivo físico. Para ello se toman medidas a distintas separaciones conocidas, se comparan con la distancia calculada a partir de la caja delimitadora y se analiza cómo crece el error cuando el objetivo aparece cada vez más pequeño en la imagen. Esta prueba permite comprobar el rango práctico de uso del sistema de percepción antes de plantear maniobras reales de seguimiento completo.
...
Para comparar las distintas pruebas de esta sección se emplearon métricas asociadas a tres aspectos del sistema: la calidad del detector visual, la respuesta de los controladores y la continuidad del seguimiento del objetivo. De este modo, no solo se analiza si el UAV objetivo se detecta correctamente, sino también si las señales generadas por la percepción son suficientemente estables para alimentar el lazo de control.

\vspace{0.3cm}

En la selección del modelo se utilizaron métricas de detección. El número de parámetros indica el tamaño del modelo y, por tanto, su coste aproximado de memoria y cálculo. El tiempo de inferencia se calculó como el tiempo medio necesario para procesar una imagen:

\begin{equation}
t_{\mathrm{inf}} = \frac{1}{N}\sum_{i=1}^{N} t_i,
\end{equation}

donde $t_i$ es el tiempo de inferencia del frame $i$ y $N$ es el número total de imágenes evaluadas. Para la precisión del detector se empleó la métrica \textit{mean Average Precision}. A partir de la curva precisión--recobrado de cada clase se obtiene el área bajo la curva, denominada $AP$. La métrica $\mathrm{mAP}_{50}$ evalúa esta precisión considerando como correcta una detección cuando la intersección sobre unión, o \textit{IoU}, supera el umbral de 0.50:

\begin{equation}
\mathrm{mAP}_{50} = \frac{1}{C}\sum_{c=1}^{C} AP_c(0.50),
\end{equation}

donde $C$ es el número de clases. En este proyecto la clase de interés es el UAV objetivo, por lo que esta métrica resume la calidad de detección de dicho objeto. La métrica $\mathrm{mAP}_{50-95}$ es más exigente, ya que promedia el $AP$ para distintos umbrales de \textit{IoU} entre 0.50 y 0.95:

\begin{equation}
\mathrm{mAP}_{50-95} = \frac{1}{10C}\sum_{\tau \in \{0.50,0.55,\ldots,0.95\}}\sum_{c=1}^{C} AP_c(\tau).
\end{equation}

Esta última métrica penaliza más las cajas delimitadoras mal ajustadas, por lo que resulta útil para valorar si el detector puede proporcionar medidas suficientemente precisas para estimar el centro del objetivo y su distancia aparente.

\vspace{0.3cm}

En las pruebas de control se analizaron los errores visuales y de distancia. Si $(u_i,v_i)$ representa el centro de la caja detectada en el frame $i$, y $W$ y $H$ son la anchura y la altura de la imagen, los errores normalizados de centrado horizontal y vertical se expresan como:

\begin{equation}
E_{x,i}=\frac{u_i-W/2}{W/2}, \qquad E_{y,i}=\frac{v_i-H/2}{H/2}.
\end{equation}

El error $E_x$ se asocia al PID de \textit{yaw}, ya que mide el desplazamiento horizontal del objetivo en la imagen. El error $E_y$ se asocia al PID de altura, ya que mide el desplazamiento vertical respecto al centro de la cámara. En ambos casos se trabaja con magnitudes normalizadas, de forma que valores próximos a cero indican que el objetivo se encuentra cerca del centro de la imagen.

\vspace{0.3cm}

En las pruebas completas de seguimiento, el error de centrado en imagen se resumió mediante el \textit{End-Point Error} bidimensional, o EPE 2D. Esta métrica agrupa los errores horizontal y vertical en una única magnitud normalizada:

\begin{equation}
\mathrm{EPE}_i = \sqrt{E_{x,i}^{2}+E_{y,i}^{2}}.
\end{equation}

Para analizar correctamente la calidad del seguimiento, el EPE se separó en función del estado de la percepción. Sea $\mathcal{I}_{\mathrm{YOLO}}$ el conjunto de frames con detección directa, $\mathcal{I}_{\mathrm{Kalman}}$ el conjunto de frames en los que se usa la predicción del filtro de Kalman y $\mathcal{I}_{\mathrm{V}}=\mathcal{I}_{\mathrm{YOLO}}\cup\mathcal{I}_{\mathrm{Kalman}}$ el conjunto de frames con referencia visual válida. Entonces:
...
Cuando no se emplea Kalman durante una prueba, el valor $\mathrm{EPE}_{\mathrm{Kalman}}$ no se calcula y se indica como no aplicable. Esta separación permite saber si el error de centrado procede de detecciones directas de YOLO o de intervalos en los que el sistema mantiene el objetivo mediante predicción.

\vspace{0.3cm}

Para el eje longitudinal se utilizó el error de distancia respecto a la separación deseada:

\begin{equation}
E_{z,i}=d_i-d_{\mathrm{ref}},
\end{equation}

donde $d_i$ es la distancia estimada en el instante $i$ y $d_{\mathrm{ref}}$ es la distancia de seguimiento fijada como referencia. A diferencia de $E_x$ y $E_y$, este error se expresa en metros. En las pruebas de seguimiento, esta componente se resumió mediante el RMSE de distancia calculado sobre los frames con referencia válida:
...
El RMSE penaliza más los errores grandes que los pequeños, por lo que permite detectar si durante la prueba aparecen desviaciones importantes aunque la respuesta media parezca aceptable. En el análisis se interpreta cada eje por separado, ya que los errores de imagen están normalizados mientras que el error longitudinal se expresa en metros.

\vspace{0.3cm}

En las pruebas específicas de los PID también se empleó el tiempo de establecimiento, definido como el tiempo necesario para que la señal de error entre en una banda de tolerancia alrededor de la referencia y permanezca dentro de ella:

\begin{equation}
T_s = \min\{t_s: |e(t)| \leq \varepsilon,\; \forall t \geq t_s\}.
\end{equation}

Esta métrica permite comparar la rapidez de respuesta de cada controlador. Un valor bajo de $T_s$ indica que el sistema alcanza la zona de referencia rápidamente, mientras que un valor alto refleja una respuesta más lenta o una dinámica más conservadora.

\vspace{0.3cm}

Por último, en las pruebas de seguimiento se evaluó la continuidad de la percepción mediante métricas de visión. Sea $N$ el número total de frames analizados, $N_{\mathrm{YOLO}}$ el número de frames en los que el detector localiza directamente el objetivo, $N_{\mathrm{Kalman}}$ el número de frames en los que se recurre al seguimiento predictivo del filtro de Kalman y $N_{\mathrm{perdidos}}$ el número de frames sin una referencia válida. Entonces:

\begin{equation}
P_{\mathrm{YOLO}}=\frac{N_{\mathrm{YOLO}}}{N}\cdot100, \qquad
P_{\mathrm{Kalman}}=\frac{N_{\mathrm{Kalman}}}{N}\cdot100,
\end{equation}
...
El porcentaje $P_{\mathrm{YOLO}}$ representa la proporción de frames con detección directa, mientras que $P_{\mathrm{Kalman}}$ indica cuántas veces fue necesario mantener temporalmente el seguimiento mediante predicción. La métrica \textit{Target Loss Rate} (TLR) mide el porcentaje de frames en los que el sistema pierde completamente la referencia del objetivo. Por tanto, un TLR bajo es deseable, y un valor de $0\%$ indica que no se produjo pérdida visual durante la prueba.

\vspace{0.3cm}

La división entre frames con YOLO, frames con Kalman y frames perdidos es importante para no mezclar errores de naturaleza distinta. Cuando el sistema no dispone de una caja válida, el código marca la medida con un valor prefijado, como $-999.0$, para indicar que no existe una observación fiable. Ese valor no representa un error real del PID, sino una pérdida de percepción. Si se incluyera directamente en la media, unos pocos frames sin detección podrían dominar toda la estadística: por ejemplo, un vuelo con el objetivo casi centrado durante la mayor parte del tiempo quedaría penalizado si durante una oclusión breve se promediaran varios valores $-999.0$. Por este motivo, la calidad del control visual se evalúa con EPE y RMSE únicamente sobre frames válidos, mientras que las pérdidas de percepción se cuantifican por separado mediante el TLR.
...
Esta subsección recoge las pruebas destinadas a comprobar el comportamiento del sistema cuando el UAV presa realiza trayectorias completas de seguimiento. A diferencia de las validaciones individuales de los PID, en estas pruebas se analiza el funcionamiento integrado de percepción, estimación y control durante un vuelo continuo. El bloque no se limita a una única maniobra, sino que reúne tres escenarios progresivos: una trayectoria en zigzag, una trayectoria circular ondulada y una ruta final definida por \textit{waypoints}.

\vspace{0.3cm}

La primera prueba corresponde a una trayectoria en zigzag. Esta maniobra resulta adecuada para evaluar el seguimiento visual porque obliga al UAV cazador a corregir cambios laterales sucesivos y alternos, manteniendo al mismo tiempo la distancia de referencia y el centrado vertical del objetivo. Por tanto, se trata de una prueba más exigente que un desplazamiento rectilíneo, ya que introduce variaciones frecuentes en el error horizontal y permite comprobar si el sistema mantiene la referencia visual sin depender de una trayectoria suave y constante.

\vspace{0.3cm}

La segunda prueba reutiliza la trayectoria circular ondulada empleada en la validación conjunta de los tres PID. En este caso, el objetivo combina desplazamiento circular en el plano horizontal con variaciones de altura, por lo que permite evaluar el seguimiento cuando aparecen simultáneamente errores de \textit{yaw}, altura y distancia. Esta prueba sirve como puente entre la validación de control y la evaluación de continuidad visual mediante las métricas EPE/RMSE/TLR.

\vspace{0.3cm}

La tercera prueba corresponde a la ruta final por \textit{waypoints}. Esta trayectoria introduce tramos de mayor longitud, cambios de dirección y variaciones de altura entre puntos consecutivos, generando una maniobra tridimensional menos regular que las anteriores. Su objetivo es comprobar si el sistema conserva la referencia visual y mantiene errores acotados en un escenario de seguimiento más general y representativo.

\subsubsection{Prueba de seguimiento con trayectoria en zigzag}

En esta prueba, el UAV presa realiza un desplazamiento en zigzag mientras el UAV cazador intenta mantenerlo dentro del campo de visión. Durante el vuelo permanecen activos los tres controladores principales: el PID de \textit{yaw}, encargado de corregir el error horizontal de la imagen; el PID de altura, responsable de reducir el error vertical; y el PID de distancia, utilizado para regular la separación longitudinal respecto al objetivo.
...
Además de la trayectoria en zigzag, se reutilizó la trayectoria circular ondulada empleada en la prueba conjunta de los tres PID para obtener métricas específicas de seguimiento. Esta trayectoria, mostrada previamente en la Figura \ref{fig:trayectoria-total-pid}, combina desplazamiento circular en el plano horizontal con variaciones sinusoidales de altura, por lo que permite evaluar la continuidad del seguimiento cuando el objetivo genera errores simultáneos en \textit{yaw}, altura y distancia.

\vspace{0.3cm}

El uso de esta misma trayectoria permite conectar la validación de control con la evaluación de seguimiento. En la prueba conjunta de PID se comprobó que los tres reguladores podían actuar de forma simultánea sin provocar una respuesta inestable; en este segundo análisis se estudia, además, si el sistema de percepción mantiene una referencia visual suficientemente continua durante toda la maniobra. Para ello se recogieron las métricas EPE/RMSE/TLR del vuelo y se registraron tanto el error de centrado bidimensional como el error de distancia y los porcentajes de detección directa, seguimiento mediante Kalman y pérdida del objetivo.
...
Como última prueba de seguimiento se definió una trayectoria formada por una secuencia de \textit{waypoints}. A diferencia de las trayectorias anteriores, esta ruta no sigue una forma periódica simple, sino que combina desplazamientos largos en el plano horizontal con cambios de altura entre puntos consecutivos. En esta versión, la ruta aumenta el recorrido longitudinal y presenta un tramo final más prolongado, por lo que el UAV presa realiza una maniobra más exigente en la que el UAV cazador debe mantener la referencia visual mientras el objetivo cambia simultáneamente su posición lateral, longitudinal y vertical.

\vspace{0.3cm}

Los puntos utilizados para generar la trayectoria se recogen en la Tabla \ref{tab:waypoints-ruta-final}. Cada \textit{waypoint} está definido mediante sus coordenadas $(x,y,z)$ en metros dentro del entorno de simulación.
...
Esta prueba se plantea como una primera validación del sistema de estimación de distancia utilizando la cámara real y el UAV objetivo real. A diferencia de las pruebas realizadas en simulación, en este caso se emplea el montaje físico del sistema, de forma que la estimación depende de la calibración real de la cámara, de las dimensiones medidas del dron objetivo y de las variaciones propias de una detección visual en un entorno no ideal.

\vspace{0.3cm}

El objetivo principal de esta prueba es comprobar si la distancia calculada a partir de la caja delimitadora mantiene una relación coherente con la separación real entre la cámara y el dron objetivo. Para ello se realizarán capturas a distintas distancias, comenzando por separaciones cercanas, donde el UAV ocupa una parte significativa de la imagen, y aumentando progresivamente la separación hasta distancias mayores. De este modo se podrá analizar cómo evoluciona la estimación cuando el tamaño aparente del dron disminuye y la detección se vuelve más sensible a pequeños errores en la caja.

\vspace{0.3cm}

Además de evaluar la precisión a corta distancia, esta prueba permitirá estudiar el límite práctico del sistema de percepción. En particular, se buscará determinar hasta qué distancia la cámara y el detector son capaces de localizar el UAV objetivo de forma suficientemente estable como para estimar su profundidad y mantener una referencia útil para el seguimiento. Esta información resulta importante para las futuras pruebas reales de persecución, ya que define el rango aproximado en el que el dron cazador puede detectar, estimar la distancia y comenzar a aproximarse al objetivo con seguridad.

\vspace{0.3cm}

El montaje experimental empleado para esta prueba se muestra en la Figura \ref{fig:dron-prueba-distancia}. En él se situó el dron objetivo frente al equipo de captura y se utilizó un metro como referencia física para ir colocando el UAV a distintas separaciones conocidas. De esta forma fue posible comparar la distancia real medida manualmente con la distancia estimada por el sistema.
...
A partir de las mediciones realizadas se generó la gráfica de error de la Figura \ref{fig:grafica-error-distancia-real}. En ella se representa, para cada distancia estimada, el error de estimación expresado en centímetros. Esta representación permite observar cómo evoluciona la precisión del método cuando aumenta la separación entre la cámara y el UAV objetivo.

\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{imagenes/Grafica_error_dist.png}
\caption{Error de estimación de distancia obtenido con la cámara real y el UAV objetivo real. (Imagen de Elaboración Propia)}
\label{fig:grafica-error-distancia-real}
\end{figure}

\vspace{0.2cm}

Los resultados muestran que, para distancias cortas y medias, el error se mantiene reducido en comparación con el rango total de medida. En la zona inicial de la gráfica aparecen pequeñas desviaciones positivas y negativas, lo que indica que el sistema puede sobreestimar o subestimar ligeramente la distancia, pero sin perder una relación coherente entre el tamaño aparente del dron y la separación real. Este comportamiento es esperable, ya que a corta distancia la caja delimitadora ocupa más píxeles y la estimación basada en anchura.

\vspace{0.2cm}

A medida que aumenta la distancia, el error crece de forma más clara. En la parte final de la gráfica, para estimaciones superiores a varios metros, se observan errores del orden de decenas de centímetros, llegando aproximadamente a valores próximos a $55\,\mathrm{cm}$. Esto se debe a que el UAV aparece cada vez más pequeño en la imagen: una diferencia de pocos píxeles en la anchura o en la altura de la caja representa una variación mayor en la distancia calculada. Además, a distancias largas influyen más la resolución de la cámara, la estabilidad de la detección, la orientación del dron y la calidad de la calibración.

\vspace{0.2cm}

En conjunto, la prueba confirma que el sistema de estimación resulta útil para trabajar en rangos cercanos y medios, que son los más relevantes para iniciar y mantener un seguimiento visual seguro. No obstante, también evidencia que la estimación pierde precisión progresivamente a distancias altas. Por tanto, para pruebas reales de persecución conviene interpretar la distancia estimada como una referencia aproximada de control y no como una medida métrica exacta, especialmente cuando el objetivo se encuentra lejos y ocupa pocos píxeles en la imagen.
...
El conjunto de pruebas realizado permite valorar en qué medida se han cumplido los objetivos de validación planteados para el sistema. Para ello, los resultados se analizan diferenciando entre percepción, control, seguimiento en simulación y estimación de distancia con cámara real. Esta separación es importante porque el sistema no depende de un único componente: una detección visual precisa no garantiza por sí sola un buen seguimiento si el control de distancia no responde adecuadamente, y un controlador estable tampoco es suficiente si la referencia visual se pierde durante el vuelo.

\vspace{0.2cm}

En la Tabla \ref{tab:analisis-cumplimiento-resultados} se resume el grado de cumplimiento de los principales puntos evaluados durante el capítulo. Se considera que un punto se ha cumplido cuando los resultados obtenidos satisfacen el objetivo principal de la prueba. Se considera cumplimiento parcial cuando el sistema funciona, pero aparecen limitaciones relevantes que deben tenerse en cuenta. Finalmente, se considera no cumplido o pendiente cuando la prueba evidencia una limitación que no queda completamente resuelta en este trabajo.
...
\hline
Selección del detector visual & Cumplido & YOLOv8n ofrece un compromiso adecuado entre precisión, tamaño e inferencia, con \textit{mAP50} de 0.992507, \textit{mAP50-95} de 0.733692 y 2.660333 ms de inferencia. \\
Control de \textit{yaw} & Cumplido & El error horizontal se corrige de forma rápida, con un tiempo de establecimiento aproximado de $0.65\,\mathrm{s}$. \\
...
Control de distancia & Cumplido parcialmente & El controlador mantiene una separación segura, pero presenta una respuesta más lenta, con $T_s \approx 6\,\mathrm{s}$, y errores de distancia superiores a los errores de centrado visual. \\
Seguimiento visual en simulación & Cumplido & En las trayectorias evaluadas el TLR fue del $0.00\%$, con detección directa de YOLO igual o superior al $99.74\%$. \\
Seguimiento en zigzag & Cumplido parcialmente & La referencia visual se mantiene durante toda la prueba, pero el RMSE de distancia alcanza $1.255\,\mathrm{m}$, mostrando mayor dificultad en la separación longitudinal. \\
...
Ruta final por \textit{waypoints} & Cumplido & El sistema mantiene un TLR nulo, detección YOLO del $99.98\%$ y EPE medio total de 0.0221 en una trayectoria tridimensional menos regular. \\
Estimación de distancia con cámara real & Cumplido parcialmente & La estimación resulta útil a distancias cortas y medias, pero pierde precisión a distancias altas, con errores que llegan a valores próximos a $55\,\mathrm{cm}$. \\
...
Desde el punto de vista de la percepción, el objetivo principal se considera cumplido. El torneo de modelos permitió seleccionar YOLOv8n como detector principal, no por ser el modelo con mayor precisión absoluta, sino por ofrecer la mejor relación entre precisión e inferencia. Esta decisión queda reforzada por las pruebas de seguimiento, donde el detector mantuvo porcentajes de visión activa muy altos: $100.00\%$ en zigzag, $99.74\%$ en la trayectoria circular ondulada y $99.98\%$ en la ruta final por \textit{waypoints}. Además, el TLR fue siempre del $0.00\%$, por lo que no se produjo pérdida completa del UAV objetivo en ninguna trayectoria de simulación.

\vspace{0.3cm}

En cuanto al control, los objetivos se cumplen de forma clara en los ejes de \textit{yaw} y altura. El controlador de \textit{yaw} fue el más rápido, con un tiempo de establecimiento de $0.65\,\mathrm{s}$, lo que confirma que el sistema puede corregir de forma eficaz desplazamientos horizontales del objetivo en la imagen. El controlador de altura también mostró un comportamiento satisfactorio, con $T_s \approx 1.25\,\mathrm{s}$ y una respuesta estable. Estos dos resultados indican que el centrado visual del objetivo en imagen se resuelve de forma adecuada en los ejes lateral y vertical.

\vspace{0.2cm}

El punto que se cumple solo parcialmente es el control de distancia. El PID longitudinal consigue mantener una separación de seguimiento segura y evita una aproximación agresiva al objetivo, pero su respuesta es más lenta que la de los otros dos controladores, con un tiempo de establecimiento aproximado de $6\,\mathrm{s}$. Esta limitación también aparece en las pruebas de seguimiento: en la trayectoria en zigzag el RMSE de distancia fue de $1.255\,\mathrm{m}$, superior al de la trayectoria circular ondulada y al de la ruta final. Por tanto, el sistema cumple el objetivo de mantener el seguimiento sin perder el objetivo, pero no alcanza la misma precisión longitudinal que en el centrado visual.

\vspace{0.2cm}

Las pruebas de seguimiento en simulación muestran un cumplimiento global positivo. La trayectoria en zigzag permitió comprobar que el sistema soporta cambios laterales sucesivos sin perder la referencia visual, aunque evidenció la dificultad del eje de distancia. La trayectoria circular ondulada validó el funcionamiento simultáneo de los tres PID en una maniobra tridimensional, con un RMSE de distancia reducido de $0.2200\,\mathrm{m}$. Finalmente, la ruta por \textit{waypoints} confirmó la robustez del sistema en una trayectoria menos regular, con TLR nulo y un EPE medio total de 0.0221. Por tanto, el seguimiento visual y el centrado del objetivo se consideran cumplidos en simulación.

\vspace{0.2cm}

Respecto a las pruebas reales, el objetivo se cumple de forma parcial. La estimación de distancia con cámara real mantiene una relación coherente con la separación física para rangos cercanos y medios, lo que demuestra que el método basado en la caja delimitadora puede proporcionar una referencia útil para el control. Sin embargo, a medida que aumenta la distancia, el objetivo ocupa menos píxeles en la imagen y pequeñas variaciones de la caja producen errores mayores. Por este motivo, la estimación de distancia no debe interpretarse como una medida métrica exacta a largas distancias, sino como una referencia aproximada para iniciar y mantener el seguimiento.

\vspace{0.3cm}

En conjunto, puede afirmarse que se han cumplido los objetivos principales relacionados con la selección del detector, el centrado visual del objetivo, la continuidad de la percepción y el seguimiento en simulación. Los puntos que no quedan completamente cumplidos son la precisión del control longitudinal en maniobras exigentes, la exactitud de la estimación de distancia a largas distancias y la validación de una persecución real completa con ambos UAV en vuelo. Estas limitaciones no invalidan el sistema desarrollado, pero sí marcan las líneas de mejora más importantes para trabajos futuros: ajustar el control de distancia, mejorar la estimación de profundidad y trasladar progresivamente las pruebas de seguimiento desde simulación a un entorno real controlado.
