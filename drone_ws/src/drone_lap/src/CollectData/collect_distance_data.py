...
class DistanceDataCollector:
    ...
    def yolo_cb(self, msg):
        if msg.z > msg.x:
            ...
            self.yolo_bbox_area = (yolo_bbox_width * yolo_bbox_height) / (CAM_WIDTH * CAM_HEIGHT) #(yolo_bbox_width * yolo_bbox_height)
    ...
    def process_data_cb(self, event):
        if self.pose_d1 and self.pose_d2 and self.yolo_detected:
            ...
            rospy.loginfo(f"Capturado -> Z_depth: {Z_depth:.2f}m | Area_norm: {self.yolo_bbox_area:.5f} (Muestras: {len(self.data_list)})")
